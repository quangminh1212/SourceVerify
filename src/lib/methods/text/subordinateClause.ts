...
export function analyzeSubordinateClause(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "Subordinate Clause", nameKey: "signal.subordinateClause", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.subordinateClause.error", icon: "🔀" };
    }
    ...
    return {
        ...
        description: score > 55 ? "Subordinate clause frequency — pattern suggests AI generation" : "Natural subordinate clause frequency — consistent with human writing",
