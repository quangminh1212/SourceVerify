...
export function analyzeConjunctionDensity(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "Conjunction Density", nameKey: "signal.conjunctionDensity", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.conjunctionDensity.error", icon: "🔗" };
    }
    ...
    return {
        ...
        description: score > 55 ? "Conjunction usage pattern — pattern suggests AI generation" : "Natural conjunction usage pattern — consistent with human writing",
