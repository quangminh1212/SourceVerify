...
export function analyzeAdverbFrequency(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "Adverb Frequency", nameKey: "signal.adverbFrequency", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.adverbFrequency.error", icon: "📝" };
    }
    ...
    return {
        ...
        description: score > 55 ? "Adverb usage pattern analysis — pattern suggests AI generation" : "Natural adverb usage pattern analysis — consistent with human writing",
