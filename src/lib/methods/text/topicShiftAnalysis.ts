...
export function analyzeTopicShiftAnalysis(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "Topic Shift Analysis", nameKey: "signal.topicShiftAnalysis", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.topicShiftAnalysis.error", icon: "🔄" };
    }
    ...
    return {
        ...
        description: score > 55 ? "Topic transition pattern — pattern suggests AI generation" : "Natural topic transition pattern — consistent with human writing",
