...
export function analyzeNegationPattern(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "Negation Pattern", nameKey: "signal.negationPattern", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.negationPattern.error", icon: "🚫" };
    }
    ...
    return {
        ...
        description: score > 55 ? "Negation usage analysis — pattern suggests AI generation" : "Natural negation usage analysis — consistent with human writing",
