...
export function analyzeSentenceConnectivity(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "Sentence Connectivity", nameKey: "signal.sentenceConnectivity", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.sentenceConnectivity.error", icon: "🔗" };
    }
    ...
    return {
        ...
        description: score > 55 ? "Sentence connectivity analysis — pattern suggests AI generation" : "Natural sentence connectivity analysis — consistent with human writing",
