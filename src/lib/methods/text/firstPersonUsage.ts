/**
 * First Person Usage
 * First person perspective pattern
 */
...
export function analyzeFirstPersonUsage(text: string): AnalysisMethod {
    ...
    if (sentences.length < 3) {
        return { name: "First Person Usage", nameKey: "signal.firstPersonUsage", category: "statistical", score: 50, weight: 0.2, description: "Too few sentences", descriptionKey: "signal.firstPersonUsage.error", icon: "👤" };
    }
    ...
    return {
        ...
        description: score > 55 ? "First person perspective pattern — pattern suggests AI generation" : "Natural first person perspective pattern — consistent with human writing",
