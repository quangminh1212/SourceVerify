...
export function analyzeZipfLaw(pixels: Uint8ClampedArray, width: number, height: number): AnalysisMethod {
    ...
    if (sorted.length < 10) {
        return {
            ...
            descriptionKey: "signal.zipf.error", icon: "â„¤",
        };
    }
    ...
    // Natural images: slope â‰ˆ -1 to -1.5, high RÂ²
    ...
    return {
        ...
        description: score > 55
            ? "Intensity distribution deviates from Zipf's law â€” potential AI generation artifact"
            : "Intensity distribution follows natural Zipf-like pattern",
        descriptionKey: score > 55 ? "signal.zipf.ai" : "signal.zipf.real",
        icon: "â„¤",
        details: `Zipf slope: ${slope.toFixed(3)}, RÂ²: ${rSquared.toFixed(4)}.`,
    };
}
...
export function analyzeMarkovTransition(pixels: Uint8ClampedArray, width: number, height: number): AnalysisMethod {
    ...
    if (totalTransitions < 100) {
        return {
            ...
            descriptionKey: "signal.markov.error", icon: "â‡Œ",
        };
    }
    ...
    return {
        ...
        description: score > 55
            ? "Pixel transitions show abnormal smoothness â€” AI images have over-correlated pixels"
            : "Pixel transition patterns appear natural â€” consistent with real image content",
