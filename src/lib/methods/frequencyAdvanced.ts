...
export function analyzeRadialSpectrum(pixels: Uint8ClampedArray, width: number, height: number): AnalysisMethod {
    ...
    if (logProfile.length < 5) {
        return {
            ...
            descriptionKey: "signal.radial.error", icon: "◎",
        };
    }
    ...
    return {
        ...
        category: "frequency", score, weight: 0.4,
        description: score > 55
            ? "Radial power spectrum shows atypical frequency distribution — potential AI signature"
            : "Radial power spectrum shows natural frequency falloff — consistent with camera optics",
