...
export function analyzeGradOrientHist(p: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    ...
    const bins=new Float64Array(8);let cnt=0;const step=3;for(let y=1;y<h-1;y+=step)for(let x=1;x<w-1;x+=step){const i=(y*w+x)*4;const gx=p[i+4]-p[i-4],gy=p[i+w*4]-p[i-w*4];const mag=Math.sqrt(gx*gx+gy*gy);if(mag>5){let ang=Math.atan2(gy,gx)+Math.PI;const bin=Math.floor(ang/(2*Math.PI)*8)%8;bins[bin]++;cnt++;}}let mx=0,mn=Infinity;for(let i=0;i<8;i++){if(bins[i]>mx)mx=bins[i];if(bins[i]<mn)mn=bins[i];}const u=mx>0?mn/mx:1;
    let score: number;
