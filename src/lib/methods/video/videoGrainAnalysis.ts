...
export function analyzeVideoGrainAnalysis(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Video Grain", nameKey: "signal.videoGrainAnalysis", category: "statistical", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.videoGrainAnalysis.error", icon: "🎞" };
    }
