...
export function analyzeTeethConsistency(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Teeth Consistency", nameKey: "signal.teethConsistency", category: "statistical", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.teethConsistency.error", icon: "🦷" };
    }
