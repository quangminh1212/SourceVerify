...
export function analyzeEdgeRinging(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Edge Ringing", nameKey: "signal.edgeRinging", category: "statistical", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.edgeRinging.error", icon: "〰" };
    }
let ring=0,total=0;
