...
export function analyzeChromaBleed(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Chroma Bleed", nameKey: "signal.chromaBleed", category: "statistical", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.chromaBleed.error", icon: "🌈" };
    }
let bleed=0,total=0;
