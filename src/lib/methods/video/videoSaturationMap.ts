...
export function analyzeVideoSaturationMap(p: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    ...
    let sats=[];const step=4;for(let y=0;y<h;y+=step)for(let x=0;x<w;x+=step){const i=(y*w+x)*4;const mx=Math.max(p[i],p[i+1],p[i+2]),mn=Math.min(p[i],p[i+1],p[i+2]);sats.push(mx>0?(mx-mn)/mx:0);}const avg=sats.reduce((a,b)=>a+b,0)/sats.length;const cv=Math.sqrt(sats.reduce((a,b)=>a+(b-avg)**2,0)/sats.length)/(avg||1);
    let score: number;
