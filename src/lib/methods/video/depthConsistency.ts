...
export function analyzeDepthConsistency(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Depth Consistency", nameKey: "signal.depthConsistency", category: "statistical", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.depthConsistency.error", icon: "🔭" };
    }
