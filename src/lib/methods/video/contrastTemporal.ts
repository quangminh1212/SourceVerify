...
export function analyzeContrastTemporal(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Contrast Temporal", nameKey: "signal.contrastTemporal", category: "statistical", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.contrastTemporal.error", icon: "🔲" };
    }
let minV=255,maxV=0;
