...
export function analyzeFrameEnergy(pixels: Uint8ClampedArray, w: number, h: number): AnalysisMethod {
    if (w < 16 || h < 16) {
        return { name: "Frame Energy Distribution", nameKey: "signal.frameEnergy", category: "forensic", score: 50, weight: 0.2, description: "Frame too small", descriptionKey: "signal.frameEnergy.error", icon: "⚡" };
    }
    const blockSize = 8;
    ...
    return {
        ...
        description: score > 55 ? "Frame energy distribution analysis — potential AI-generated video artifact" : "Natural frame energy distribution analysis — consistent with authentic video",
