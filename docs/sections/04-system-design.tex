...
\texttt{AnalysisResult} & Điểm AI cuối, nhãn (ai/real/uncertain), độ tin cậy, danh sách tín hiệu \\
...
Ngưỡng cuối cùng đặt như sau: điểm $\geq 55$ là \texttt{ai}, $\leq 40$ là \texttt{real}, còn lại là \texttt{uncertain}. Em để một vùng "chưa chắc" ở giữa thay vì ép buộc phải chọn ai hoặc real, vì với những ảnh không đủ bằng chứng thì trả lời "chưa rõ" trung thực hơn là đoán bừa. Độ tin cậy được tính dựa trên khoảng cách của điểm tới ngưỡng: điểm càng xa vùng giữa thì hệ thống càng chắc chắn. Ví dụ ảnh được 90 điểm sẽ có độ tin cậy cao hơn nhiều so với ảnh chỉ được 57 điểm, dù cả hai đều bị xếp là \texttt{ai}.
