...
function calculateVerdict(signals) {
    ...
    for (const s of signals) {
        if (s.score > 55) aiLeaningWeight += s.weight;  // raised from 50
        if (s.score < 45) realLeaningWeight += s.weight; // lowered from 50
        if (s.score >= 70) strongAI++;     // raised from 65
        if (s.score <= 30) strongReal++;   // lowered from 35
        if (s.score >= 82) veryStrongAI++; // raised from 78
        if (s.score <= 18) veryStrongReal++;// lowered from 22
    }
    let adjustment = 0;
    // Consensus - more conservative
    ...
    // Weighted majority - reduced from 14 to 8
    const weightRatio = totalWeight > 0 ? (aiLeaningWeight - realLeaningWeight) / totalWeight : 0;
    adjustment += Math.round(weightRatio * 8);

    // Directional amplification - much weaker
    const deviation = aiScore - 50;
    if (Math.abs(deviation) > 3) {
        const linear = deviation * 0.5;  // was 1.1
        const quadratic = Math.sign(deviation) * (deviation * deviation) * 0.008; // was 0.025
    ...
    // Verdict - balanced thresholds
    let verdict, confidence;
    if (aiScore >= 56) { verdict = "ai"; confidence = Math.min(100, Math.round(50 + (aiScore - 56) * 1.1)); }
    else if (aiScore <= 44) { verdict = "real"; confidence = Math.min(100, Math.round(50 + (44 - aiScore) * 1.1)); }
    else { verdict = "uncertain"; confidence = Math.round(100 - Math.abs(aiScore - 50) * 6); }
